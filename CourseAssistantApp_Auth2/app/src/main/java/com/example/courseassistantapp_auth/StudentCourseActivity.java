...
public class StudentCourseActivity extends AppCompatActivity {
        ...
        if (auth.getCurrentUser() != null) {
            ...
            coursesReference.orderByChild("startDate").addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    ...
                    Iterator<DataSnapshot> courseIterator = dataSnapshot.getChildren().iterator();

                    while (courseIterator.hasNext()) {
                        DataSnapshot courseSnapshot = courseIterator.next();
                        DataSnapshot groupsSnapshot = courseSnapshot.child("groups");

                        boolean userInGroup = false; // flag kontrolü

                        Iterator<DataSnapshot> groupIterator = groupsSnapshot.getChildren().iterator();
                        while (groupIterator.hasNext() && !userInGroup) { // userInGroup flag'i kontrol ederek
                            DataSnapshot groupSnapshot = groupIterator.next();

                            Iterator<DataSnapshot> emailIterator = groupSnapshot.getChildren().iterator();
                            while (emailIterator.hasNext() && !userInGroup) { // iç döngüde flag kontrolü
                                DataSnapshot emailSnapshot = emailIterator.next();
                                String email = emailSnapshot.getValue(String.class);

                                if (email != null && email.equals(auth.getCurrentUser().getEmail())) {
                                    userInGroup = true;
                                }
                            }
                        }
                        ...
                        if (userInGroup) {
                            ...
                            System.out.println("buradanevar: " + courseCode);
                            if (courseCode != null) {
                    ...
                    for (DataSnapshot courseSnapshot : userCourses) {
                        ...
                        addCourseCard(courseCode, courseName, courseState); // Kartları ekle
                    }
                }

                @Override
                public void onCancelled(DatabaseError databaseError) {
                    Toast.makeText(StudentCourseActivity.this, "Error fetching courses: " + databaseError.getMessage(), Toast.LENGTH_LONG).show();
                }
            });
        } else {
            Toast.makeText(StudentCourseActivity.this, "No user logged in.", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void addCourseCard(String courseCode, String courseName, String courseState) {
        ...
        card.setOnClickListener(v -> {
            //Intent intent = new Intent(StudentCourseActivity.this, ForumActivity.class);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.student_course_menu, menu);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (id == R.id.cm_refresh) {
            ...
            Intent intent = new Intent(StudentCourseActivity.this, UserProfileActivity.class);
            startActivity(intent);

        }else if (id == R.id.cm_forum) {
            ...
            Intent intent = new Intent(StudentCourseActivity.this, PostsActivity.class);
            startActivity(intent);

        } else {
            Toast.makeText(StudentCourseActivity.this, "Something went wrong.", Toast.LENGTH_SHORT).show();
        }
